...
class IndexSelector extends React.Component
{
    ...
    createIndexElement(index)
    {
        return this.props.url
            ...
            : (
                <button key={index} className={`${styles.index} ${index === this.props.index ? styles.activeIndex : ""}`} onClick={() => this.props.onIndexChange(index)}>
                    {index}
                </button>
            );
    }
    ...
    createGoToIndexElement(min, max)
    {
        ...
        return (
            <form key={min} onSubmit={this.handleGoToIndex}>
                <input className={`${styles.index} ${styles.goToIndex}`} style={style}
                    name='goto' type='number' min={min} max={max} step={1} placeholder='...'
...
IndexSelector.propTypes = 
{
    url: (props, propName, componentName) =>
    {
        ...
        else if (props[propName] && (!(typeof props[propName] === 'string') && !props[propName] instanceof String))
        {
    ...
    onIndexChange:(props, propName, componentName) =>
    {
        ...
        else if (props[propName] && !props[propName] instanceof Function)
        {
